...
- **D-01:** **Pin tag + lock digest** — tag exact de l'image Playwright officielle (patch compris, ex. `v1.61.1-noble`, Chromium uniquement) + digest sha256 piné dans un fichier lock dédié (`qa-container.lock`) référencé par les scripts : lisible au quotidien, immuable en CI même si Microsoft re-pousse le tag. Le tag verbatim est enregistré dans chaque `QAReport.qa_container_tag`. Flags `--no-sandbox --disable-dev-shm-usage` (§7.2). **Source de vérité unique (B1)** : `qa-container.lock` est l'unique source de vérité de l'image, consommé par le script local `qa:run` ET par `qa.yml` (voir D-03) — aucune référence tag@digest en dur ailleurs. **Parité version (m1)** : un test collecté par `verify` asserte version du package npm `playwright` == version extraite du tag de `qa-container.lock`.
- **D-02:** **QA locale = script `docker run`** (ex. `qa:run`) : image lockée, repo monté en volume, zéro Playwright bare-metal sur la machine Windows — conforme à l'interdit §7.2 (rendu non pinné = flake). Le même chemin d'exécution sert à `baseline:update`. **Deps dans le conteneur (m8)** : `npm ci` exécuté DANS le conteneur ; jamais monter le `node_modules` de l'hôte Windows (binaires natifs platform-specific) ; zéro install de playwright sur l'hôte.
- **D-03:** **Workflow CI dédié `qa.yml`** : job `container:` avec l'image lockée, exécutable depuis fresh checkout, sans secrets. **`verify.yml` reste byte-identique** (doctrine D-18 Ph 2) ; les tests unitaires vitest de `src/anim-qa/` restent ramassés par `verify`. **Consommation du lock (B1)** : `qa.yml` consomme `qa-container.lock` comme unique source de vérité de l'image — step `docker run` ou env injectée lisant le lock (§7.2 autorise explicitement « job `container` (ou `docker run`) ») ; **aucune référence tag@digest en dur dans le YAML** ; `qa:run` et `qa.yml` partagent le même chemin d'invocation (chemin d'exécution local/CI unique). **Gate grep (m13)** : `qa.yml` ne contient aucune occurrence de « secrets. » (spec collectée par verify ou check CI).
- **D-04:** **Cadence flake : 1 run QA par PR, preuve 10 runs consécutifs sur push `main` + schedule nightly** — les régressions sont capteses à chaque PR, la preuve < 1 % reste continue sans alourdir chaque PR. **Trigger `workflow_dispatch` (m2)** sur `qa.yml` pour la preuve 10 runs on-demand (pré-ship-gate Ph 10).
...
- **D-07:** **Spike de calibration = première tâche livrée de la phase** : mesurer le bruit d'antialiasing du conteneur pinné (runs répétés sur l'asset de référence), `maxDiffPixels = bruit observé × marge`, écrire `thresholds.json` + `docs/qa.md` (méthode + valeurs) commités — le spike devient un artefact, pas une estimation. **Protocole (m11)** : K ≥ 10 captures de a-001 ; bruit = pixelmatch pairwise entre captures du même input ; seuil = p95 observé × marge (marge fixée ET justifiée dans `docs/qa.md`) ; **critère d'acceptation** = les seuils dérivés passent la preuve identité ×10 (D-25) ; script de spike committé, paramétré par `asset_id`, appliqué aux 11 fixtures.
- **D-08:** **Frame canonique alignée sur la règle de pose D-15 Ph 3** : recettes `enter` → frame finale (fin du marker `enter` déjà émis par `markers.ts`), recettes `loop` → frame 0. Évite les frames vides non informatives (fade/slide à frame 0 = opacité 0) et garde la cohérence avec le poster SVG statique. Frames échantillonnées = 25 % / 50 % / 75 % de la durée (§7.4 verbatim). Échec de la frame canonique = rejet immédiat sans lire les autres (économie §7.4). **Ordre figé (m4)** : `frame_hashes` = `[canonique, 25 %, 50 %, 75 %]` — invariant producteur testé + préservé au round-trip bridge ; **`canonicalFrame()` fonction partagée** consommée par le poster SVG statique, la génération baseline et le frame-walker, avec test d'égalité des trois index.

### Smoke test theming
- **D-09:** **Mécanisme dark par défaut = dotLottie `setTheme`** (`@lottiefiles/dotlottie-web` dans la page QA) — verbatim §7.9, et c'est le chemin de production Ph 8 (ADR-05 `themeId` + `theme_anchors`) : la QA prouve le vrai mécanisme, EXP-05 le re-testera tel quel. Nouveau dep npm → gate `checkpoint:human-verify` (pattern svgo+tsx Ph 3). **Research gate au planning (B4)** : vérifier l'API `setTheme` de `@lottiefiles/dotlottie-web` contre le Lottie canonique Ph 3 (`nm`=anchors) + thème runtime (`dark-theme.json`) ; **fallback sans impasse** = packaging `.dotlottie` déterministe dans la QA (livrable réutilisé Ph 8, `dotlottie_sha256`) — aucun scénario ne produit de travail jetable.
- **D-10:** **Couleurs du thème dark = fixture QA dédiée versionnée** (`fixtures/qa/dark-theme.json`), choisies pour la QA uniquement, explicitement **non-produit** — respecte D-12 Ph 2 (le mapping anchor→couleur reste l'affaire de la Phase 8). Le smoke est un test d'effet (ça change visiblement), pas un test de couleurs justes. **Schéma validé par spec (m6)** : clés ⊆ vocabulaire clos des ancres (`ThemeAnchorId`), couleurs à contraste fort (luminances inversées), couverture de toutes les ancres présentes dans les 11 fixtures ; champ `scope: "qa-only"`.
...
- **D-12:** **Seuil theming agrégé** : un pourcentage sur l'**union** des régions d'anchors, comparé à 5 % (`theme:noop` si ≤) — colle au contrat `theme_diff_pct` (un seul float §7.6) ; **log non bloquant par ancre** pour l'observabilité ; la granularité par ancre reste possible en v2. **Précisions (m5)** : smoke calculé sur la **frame canonique** (D-08) ; dénominateur de `theme_diff_pct` = union des régions **non vides** (bbox sans encre dans le rendu non-thémé exclue du dénominateur et loggée) ; bbox calculées sur le rendu non-thémé de la frame comparée, appliquées identiques aux deux côtés du diff.
...
- **D-17:** **Méthode unique `anim_qa.run`** : les 5 étapes enchaînées en interne — l'ordonnancement interne reste la responsabilité du module, la gate ordonnée §7.3 est un invariant du module et non une convention d'appel. **Verdict ≠ erreur de transport (B5)** : `passed=false` → enveloppe `ok=true` avec le QAReport dans `result` ; `ok=false` réservé à l'opérationnel/protocole ; **le set clos D-28 est étendu same-commit 8→10** avec `baseline_missing` + `baseline_stale`, test de parité à jour (l'intégration Python asserte le set exact). **Arrêt sans navigateur (m7)** : échec des étapes 1–3 = JAMAIS de spawn Chromium (test d'assertion : aucun lancement browser sur input structurellement cassé).
...
- **D-19:** **Réponse = QAReport complet + pointeurs vers les artefacts écrits** (chemins des PNG frames/diff sous le répertoire de sortie passé **à l'activation du serveur**, jamais par requête) ; les PNG sont des artefacts de run, pas des données de contrat. **Pointeurs (m12)** : chemins **RELATIFS** (jamais absolus), naming déterministe `out/qa/{asset_id}/…` ; `out/qa/` gitignoré ; `qa.yml` uploade `out/qa/` en artefacts avec `if: always()`.

### Assets svg-only & routage theming
- **D-20:** **Smoke theming exécuté sur tous les assets, routé par `renderer_support`** : `all` → dotLottie `setTheme` (défaut §7.9) ; `svg-only` → **`applyTheme` déterministe sur lottie-web SVG** — le chemin de theming production exact de cette classe en Ph 8 (export React, EXP-01). Ni skip, ni échec dur, ni smoke wasm sur incompatible. **Champ additif `QAReport.theme_smoke_path: Literal["dotlottie-setTheme", "applyTheme-svg"]`** same-commit deux côtés (pattern D-16 Ph 2) — on enregistre le mécanisme mesuré, jamais « skipped ». `applyTheme` = **fonction pure née en Ph 4** avec tests unit, réutilisée telle quelle en Ph 8. Mêmes régions (masks D-11), même seuil agrégé > 5 %, même frame canonique pour les deux chemins ; le breakdown par ancre note le chemin utilisé. **Précision (m5)** : le dénominateur « régions non vides » (D-12) s'applique aux deux chemins ; bbox calculées sur le rendu non-thémé de la frame canonique, identiques des deux côtés du diff. — **Reversibility:** costly — `applyTheme` et `theme_smoke_path` seront consommés par le packager Ph 8 ; les changer après coup = migration contrats + exports.
...
- **D-23:** **Fond opaque fixe dans la `captureConfig`** (valeur tranchée : **`#ffffff`** — m9) — pas d'alpha dans les diffs : pixelmatch compare de l'opaque pur, le bruit de blending alpha disparaît, les bytes PNG sont stables.

### Frontière tests & preuves CI
- **D-24:** **Unit vitest (dans `verify`) = tout ce qui ne demande pas Chromium** : structural (zod `LottieJSONSchema` Ph 3 + feature gate + diff structurel), `diff.ts` (pixelmatch sur paires PNG synthétiques), `report.ts` (parité bridge QAReport), résolveur baseline (chemins purs), `applyTheme` (fonction pure). **Conteneur `qa.yml` = frame-walker, runner, smoke theming, preuves E2E** — le loop dev reste rapide, le navigateur ne tourne que pinné. **Ségrégation des specs (B2)** : specs conteneur = `*.qa.spec.ts`, **exclues du projet vitest par défaut** (config dédiée `vitest.qa.config.ts` invoquée par `qa.yml` dans le conteneur) ; **non collectées = non skippées** ; **assert-zero-skips s'exécute AUSSI sur le junitxml de `qa.yml`** ; les specs unit n'importent jamais playwright (**test de scan collecté par verify**).
...
- **D-26:** **Sidecar anti-baseline-périmée** : `baseline:update` écrit `baseline-frames/index.json` (lottie_sha256 par asset + version des configs QA) ; la QA compare au hash du Lottie reçu **avant tout diff** — mismatch = erreur `baseline_missing`/stale (code fermé), jamais un faux `pixel:*` ; diagnostic propre pour le retry_router Ph 7. **Prédicat stale exact (m3)** : `lottie_sha256(reçu) ≠ sidecar` **OU** `captureConfig_hash ≠ sidecar` → `baseline_stale` ; les hashs thresholds et dark-theme sont enregistrés au sidecar pour traçabilité mais **NE DÉCLENCHENT PAS stale** (appliqués au diff, pas à la capture).
...
- Marge du spike (`bruit observé × marge`, §7.5) — reste discrétionnaire mais **bornée par le protocole D-07/m11** (p95 × marge, fixée ET justifiée dans `docs/qa.md` ; critère d'acceptation = identité ×10)
...
- Champs exacts de `captureConfig` et layout du répertoire de sortie des artefacts de run — sous réserve de D-23 (`#ffffff`), D-19/m12 (naming `out/qa/{asset_id}/…`, chemins relatifs), D-26/m3 (sidecar) et D-08/m4 (`canonicalFrame()`)
- Organisation interne du `qa.yml` (jobs/steps) dans le respect de D-03/D-04 — l'upload `out/qa/` avec `if: always()` est fixé par D-19/m12
- Wording de `docs/qa.md` — sous réserve de D-07/m11 (marge fixée et justifiée)
...
### Reusable Assets
- `src/rpc/server.ts` — y ajouter la méthode `anim_qa.run` (framing NDJSON D-27 et enveloppe D-28 déjà en place ; codes fermés à étendre avec `baseline_missing` ET `baseline_stale` — 8→10, m10/B5)
